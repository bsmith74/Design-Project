...
public class PortfolioReport {
	public static void main(String args[]) {
		...
		File portfoliosInput = new File("data/Portfolios.dat");
		...
		try {
			s = new Scanner(portfoliosInput);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		int portfolioSize = Integer.parseInt(s.nextLine());
		...
		while (s.hasNextLine()) {
			...
			if (!line.trim().isEmpty()) {
				...
				if(!tokens[4].trim().isEmpty()) {
					String[] pAssets = tokens[4].split(",");
					int i = 0;
					while(!pAssets[i].trim().isEmpty()) {
						String[] assetTokens = pAssets[i].split(":");
						String assetCode = assetTokens[0];
						String assetValue = assetTokens[1];
						int j = 0;
						while()
					}
				}
